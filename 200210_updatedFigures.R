...
g <- meetings %>%
    ...
    facet_wrap(.~ date) + 
    #scale_size_manual(breaks = c(1,5,10)) +
...
g <- meetings %>%
    ...
    scale_size("Number of people", breaks = c(1,5,10), 
               guide = guide_legend(direction = "vertical")) + 
    scale_color_hue("Companies", aesthetics = c("color", "fill"), #option = "D",
...
gender <- companies_meeting %>%
    select(phase, actors, gender) %>% 
    group_by(phase, gender) %>%
    unique() %>% 
    tally() %>% 
    pivot_wider(id_cols = phase, values_from = n, 
...
df_actors %>%
    ...
    geom_point(
        data = df_actors %>% ungroup() %>% 
            ...
            unique(),
        aes(x = x, y = y, color = company , shape = gender, fill = company, size = meetings), alpha = 1) +
    scale_color_hue("Companies", aesthetics = c("color", "fill"), #option = "D",
                    guide = guide_legend(direction = "vertical", nrow = 3) ) +
    scale_size("Number of meetings",
